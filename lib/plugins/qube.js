		...
		if (isRowBased && hasFormulas) {
			...
			cubeColumns.forEach(function(col) {
				...
				body.forEach(function(row) {
					var val = numOrStr(row.values[1+col].textContent());
					values.push(val);
				});
...
function render_result(res) {	
	var copyTable = function(ctx) {
		try {
			//find the closest table
			var tbl = {};
			ctx.path.some(function(t){
				var tag = (t.tagName) ? t.tagName : '';
			...
			var hiddenArea = document.getElementById('hiddenCopyArea');
			hiddenArea.innerHTML = tbl.outerHTML;
	...
	var lnk = DOM.div({},[DOM.a({onClick:copyTable,href:'#'},[DOM.text('')])]);
	if (Array.isArray(res) && res.length > 0) {
		if (Array.isArray(res[0])) {
			...
			if (a.headers) {
				var cs = [];
				a.headers.forEach(function(h,i) {
					var isCategory = (a.dimensions.indexOf(h) >= 0);
					var isLastColumn = (i == a.headers.length-1);
					var ats = {};
					var dat = [render_result(h)];

					if(isCategory && isLastColumn) {
						ats = {className:'category lastcolumn'};
						dat.push(lnk);					
					} else if (isLastColumn) {
						ats = {className:'lastcolumn'};
						dat.push(lnk);					
					} else if (isCategory) {
					...
					attributes.push(ats);
					cs.push(DOM.th({className: ats.className},dat));
				});
				hr.push(DOM.tr({},cs));
			}
			...
			if (a.headers) {
				var cs = [];				
				a.headers.forEach(function(h, i) {
					if (i == a.headers.length-1) ats.className = 'lastcolumn';
					cs.push(DOM.th({className: ats.className},[render_result(h),lnk]));
				});
				hr.push(DOM.tr({},cs));
			}
			//TODO: slice off first 1000 and show a foot
